...
Group24Exe1Prog3
days = 1:length(normalised_deaths_uk);
...
[H1,P1] = kstest2(y1,normalised_deaths_uk);
[H2,P2] = kstest2(y2,normalised_deaths_uk);
[H3,P3] = kstest2(y3,normalised_deaths_uk);
[H4,P4] = kstest2(y4,normalised_deaths_uk);
[H5,P5] = kstest2(y5,normalised_deaths_uk);


%instead of p-values, we choose mean square error
diff1 = 0; diff2 = 0; diff3 = 0; diff4 = 0; diff5 = 0;
for i = 1:length(days)
    diff1 = diff1 + abs(normalised_deaths_uk(i)^2-y1(i)^2);
    diff2 = diff2 + abs(normalised_deaths_uk(i)^2-y2(i)^2);
    diff3 = diff3 + abs(normalised_deaths_uk(i)^2-y3(i)^2);
    diff4 = diff4 + abs(normalised_deaths_uk(i)^2-y4(i)^2);
    diff5 = diff5 + abs(normalised_deaths_uk(i)^2-y5(i)^2);
end
